...
export default function App() {
  ...
  const handleImportData = (importedData) => {
    ...
    if (importedData.currentFuel !== undefined)
      setCurrentFuel(importedData.currentFuel);
  ...
  const handleResetData = () => {
    setEntries([]);
    setCurrentFuel(0);
    // Mantemos a configuração básica (tanque, media) mas o log é zerado.
  };
    ...
    <div className="h-[100dvh] w-full bg-[#060b14] flex justify-center font-sans text-slate-200 overflow-hidden">
      ...
      <div className="h-full w-full max-w-md bg-gradient-to-b from-[#0a1325] to-[#040811] relative flex flex-col shadow-2xl md:border-x md:border-slate-800 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto pb-28 px-4 custom-scrollbar z-10 relative">
          {activeTab === "PAINEL" && (
            <TabPainel
              ...
              tanqueTotal={config.tanqueTotal}
              config={config}
            />
          )}
          {activeTab === "LOG" && <TabLog entries={entries} />}
          {activeTab === "RELATÓRIOS" && (
            <TabRelatorios entries={entries} config={config} />
          )}
          {activeTab === "CONFIGURAÇÕES" && (
            <TabConfiguracoes
              ...
              currentFuel={currentFuel}
              entries={entries}
              onCalculate={handleCalculateConfig}
          ...
          <div className="absolute bottom-24 right-4 z-40">
            {fabMenuOpen && (
              <div className="absolute bottom-16 right-0 mb-2 flex flex-col space-y-3 items-end animate-fade-in-up">
                <button
                  onClick={() => {
                    setActiveModal("despesa");
                    setFabMenuOpen(false);
                  }}
                  className="flex items-center space-x-3 bg-slate-800/90 backdrop-blur-md border border-slate-700 p-2 pr-4 rounded-full shadow-lg hover:border-red-500/50 transition-colors"
                >
                ...
                <button
                  onClick={() => {
                    setActiveModal("odometro");
                    setFabMenuOpen(false);
                  }}
                  className="flex items-center space-x-3 bg-slate-800/90 backdrop-blur-md border border-slate-700 p-2 pr-4 rounded-full shadow-lg hover:border-purple-500/50 transition-colors"
                >
                  <span className="text-sm font-medium text-slate-200">
                    Atualizar Odômetro
                  </span>
                ...
                <button
                  onClick={() => {
                    setActiveModal("abastecimento");
                    setFabMenuOpen(false);
                  }}
                  className="flex items-center space-x-3 bg-slate-800/90 backdrop-blur-md border border-slate-700 p-2 pr-4 rounded-full shadow-lg hover:border-emerald-500/50 transition-colors"
                >
                  <span className="text-sm font-medium text-slate-200">
            ...
            <button
              ...
              className={`w-14 h-14 rounded-full shadow-[0_0_15px_rgba(255,255,255,0.2)] flex items-center justify-center transition-all duration-300 z-50 ${
                fabMenuOpen
            ...
            >
              <Plus
                size={28}
                className={fabMenuOpen ? "text-white" : "text-slate-900"}
              />
            </button>
          </div>
        )}
        ...
        <nav className="absolute bottom-0 w-full h-16 bg-[#0B1221]/95 backdrop-blur-lg border-t border-cyan-900/50 grid grid-cols-5 items-center px-1 z-50 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
          ...
          <NavItem
            icon={<BarChart2 />}
            label="RELATÓRIOS"
            isActive={activeTab === "RELATÓRIOS"}
            onClick={() => setActiveTab("RELATÓRIOS")}
          />
          <NavItem
            icon={<Settings />}
            label="CONFIGS"
            isActive={activeTab === "CONFIGURAÇÕES"}
            onClick={() => setActiveTab("CONFIGURAÇÕES")}
          />
          <div className="w-full h-full"></div>
        </nav>

        {activeModal === "abastecimento" && (
          <ModalAbastecimento
            onClose={() => setActiveModal(null)}
            config={config}
            setConfig={setConfig}
            currentFuel={currentFuel}
            setCurrentFuel={setCurrentFuel}
      ...
      <style>
        {`
          ...
          /* Esconder marca d'água do CodeSandbox */
          a[href*="codesandbox"], div[style*="z-index: 9999999"] { 
            display: none !important; 
            opacity: 0 !important; 
            pointer-events: none !important;
          }
          
          /* Esconder ícone de calendário nativo no input date para manter o estilo limpo */
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full pt-10 space-y-12 animate-fade-in-up">
      <div className="relative w-full max-w-[300px] aspect-[2/1] mx-auto mt-8">
        <svg
          viewBox="0 0 200 110"
          className="w-full h-full drop-shadow-[0_0_15px_rgba(34,211,238,0.1)]"
        >
          <defs>
          ...
          >
            <polygon
              points="98.5,100 101.5,100 100,20"
              fill="#22d3ee"
              filter="url(#cyanGlow)"
            />
            <circle
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center w-full pointer-events-none">
          <div
            className="flex items-center space-x-2 text-cyan-400 mb-1"
          ...
          <span className="text-[10px] text-cyan-700 font-bold tracking-[0.2em] uppercase">
            Fuel Level
          </span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 w-full mt-16 px-2">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center backdrop-blur-sm">
          ...
          <span className="text-lg font-bold text-slate-200">
            {((currentFuel / tanqueTotal) * 100).toFixed(0)}% Cheio
          </span>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center backdrop-blur-sm">
          ...
          <span className="text-lg font-bold text-slate-200">
            ~{(currentFuel * config.kmL).toFixed(0)} km
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col space-y-4 pt-4 animate-fade-in-up">
      ...
      {sortedEntries.length === 0 ? (
        ...
        sortedEntries.map((entry) => (
          <div
            key={entry.id}
            className="bg-[#0f172a]/80 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-lg"
          >
            <div className="flex items-center space-x-4">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="font-bold text-slate-100 flex items-center flex-wrap gap-1">
                  ...
                  {entry.isFullTank && (
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 whitespace-nowrap">
                      Tanque Cheio
                    </span>
                  )}
                </span>
                <span className="text-xs text-slate-400 flex items-center mt-1">
                  ...
                  {/* Formatando a data garantindo o fuso horário correto */}
                  {new Date(entry.date).toLocaleDateString("pt-BR", {
                    timeZone: "UTC",
                  })}
                </span>
                {entry.type === "abastecimento" && (
                  <span className="text-xs text-emerald-500/80 mt-0.5">
                    +{entry.litros}L • {entry.odometro}km
                  </span>
                )}
              </div>
            </div>
              ...
              {entry.type === "abastecimento" || entry.type === "despesa" ? (
                ...
                <>
                  <span className="font-mono font-bold text-lg text-purple-400">
                    -{entry.litrosGastos.toFixed(1)}L
                  </span>
...
function TabRelatorios({ entries, config }) {
  // Novo estado para o filtro de mês (Padrão: Mês atual)
  ...
  const { consumoMedio, kmMes, gastoCombustivel, gastoManutencao, gastoGeral } =
    useMemo(() => {
      ...
      const entriesMes = entries.filter((e) => {
        // Ajuste de timezone para não cair no mês anterior dependendo da hora
        const d = new Date(e.date);
        return (
          d.getUTCMonth() === filterMonth && d.getUTCFullYear() === filterYear
        );
      });

      const abastecimentosMes = entriesMes.filter(
        (e) => e.type === "abastecimento"
      );
      const despesasMes = entriesMes.filter((e) => e.type === "despesa");

      const gastoComb = abastecimentosMes.reduce(
        (acc, curr) => acc + (curr.valorTotal || 0),
        0
      );
      const gastoManut = despesasMes.reduce(
        (acc, curr) => acc + (curr.valor || 0),
        0
      );
      const litrosTotais = abastecimentosMes.reduce(
        (acc, curr) => acc + (curr.litros || 0),
        0
      );
      ...
      // Média apenas se teve abastecimento para calcular, senão mostra a média configurada
      const media =
        litrosTotais > 0 && rodados > 0 ? rodados / litrosTotais : config.kmL;
    ...
    <div className="flex flex-col space-y-5 pt-4 animate-fade-in-up">
      <div className="flex items-center justify-between px-2">
        <h2 className="text-lg font-bold text-slate-300 flex items-center">
          <BarChart2 className="mr-2 text-cyan-500" size={20} /> Relatórios
        </h2>
        <input
          type="month"
          value={monthFilter}
      ...
      <div className="bg-gradient-to-br from-cyan-900/40 to-blue-900/20 border border-cyan-800/50 rounded-3xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col">
          <span className="text-sm font-medium text-cyan-400/80 mb-1">
            Consumo Médio Atual
          </span>
          <div className="flex items-baseline space-x-2">
            <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-emerald-300 font-mono">
              {consumoMedio}
            </span>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-[#0f172a]/80 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex flex-col justify-between h-28">
          ...
          <span className="text-xl font-bold text-slate-100 font-mono">
            {kmMes} <span className="text-xs text-slate-500">km</span>
          </span>
        </div>
        <div className="bg-[#0f172a]/80 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex flex-col justify-between h-28">
          <div className="flex justify-between items-start mb-2">
...
function TabConfiguracoes({
  ...
  const handleExportBackup = async () => {
    try {
      ...
      const fileName = `motolog_backup_${
      ...
      setBackupMsg({
        text: "Verifique a pasta de Downloads!",
        type: "success",
      });
    } catch (err) {
      console.error(err);
      setBackupMsg({
        text: "Erro ao gerar o arquivo de backup.",
        type: "error",
      });
    }
  ...
  const handleImportBackup = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setBackupMsg({
          text: "Erro ao ler arquivo. Formato incorreto.",
          type: "error",
        });
      }
  ...
  const handleWipeData = () => {
    if (confirmReset) {
      ...
      setBackupMsg({
        text: "Todos os registros foram apagados!",
        type: "success",
      });
      setTimeout(() => setBackupMsg({ text: "", type: "" }), 4000);
    } else {
      setConfirmReset(true);
      setTimeout(() => setConfirmReset(false), 3000); // Reseta o botão de confirmação após 3s
    }
  };
  ...
  return (
    <div className="flex flex-col space-y-6 pt-4 animate-fade-in-up">
      <h2 className="text-lg font-bold text-slate-300 px-2 flex items-center">
        <Settings className="mr-2 text-cyan-500" size={20} /> Parâmetros do
        Veículo
      </h2>
      <form
        onSubmit={handleSubmit}
        className="bg-[#0f172a]/80 backdrop-blur-md border border-slate-800 rounded-2xl p-5 space-y-5"
      >
        <div className="space-y-1">
          ...
          <div className="relative">
            ...
            <input
              type="number"
              name="tanqueTotal"
              value={localConfig.tanqueTotal}
              onChange={handleChange}
              step="0.1"
              className="w-full bg-[#060b14] border border-slate-700 text-slate-200 rounded-xl py-3 pl-10 pr-4 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all font-mono"
            />
          </div>
        </div>
        <div className="space-y-1">
        ...
        <button
          type="submit"
          className={`w-full mt-4 font-bold py-3.5 rounded-xl transition-all active:scale-95 flex justify-center items-center ${
            saved
              ? "bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)]"
              : "bg-teal-500 hover:bg-teal-400 text-slate-900 shadow-[0_0_15px_rgba(20,184,166,0.4)]"
          }`}
        >
          {saved ? (
            <>
              <CheckCircle2 className="mr-2" size={20} /> Salvo!
            </>
          ) : (
            "Salvar Manualmente"
          )}
      ...
      <div className="bg-[#0f172a]/80 backdrop-blur-md border border-slate-800 rounded-2xl p-5 space-y-4">
        <h3 className="text-sm font-bold text-slate-300 flex items-center border-b border-slate-800 pb-2">
          <Database className="mr-2 text-cyan-500" size={18} /> Gerenciamento de
          Dados
        </h3>
        <p className="text-xs text-slate-400 leading-relaxed">
          Exporte seus registros, importe um backup, ou zere o aplicativo para
          começar de novo.
        </p>

        {backupMsg.text && (
          <div
            className={`flex items-center p-3 rounded-lg text-xs font-medium ${
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            ...
            className="flex flex-col items-center justify-center p-3 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl transition-colors text-cyan-400 hover:text-cyan-300"
          >
        ...
        <button
          ...
          className={`w-full mt-2 flex items-center justify-center p-3 rounded-xl transition-colors border ${
            confirmReset
              ? "bg-red-600 hover:bg-red-500 text-white border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.4)]"
              : "bg-slate-900/50 hover:bg-slate-800 border-red-900/30 text-red-400"
          }`}
        >
...
function NavItem({ icon, label, isActive, onClick }) {
  return (
    ...
    >
      ...
      {isActive && (
        <div className="absolute bottom-0 w-8 h-1 bg-cyan-400 rounded-t-md shadow-[0_0_10px_#22d3ee]"></div>
      )}
    </button>
  );
}
...
function ModalAbastecimento({
  ...
  const handleSubmit = (e) => {
    ...
    // O T12:00:00Z garante que o fuso horário não jogue a data um dia para trás
    ...
    if (odometroForm > config.odometro)
      setConfig((prev) => ({ ...prev, odometro: odometroForm }));
        ...
        <div className="p-5 overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-xl font-bold text-slate-100 flex items-center">
              <Fuel className="text-emerald-500 mr-2" size={24} /> Novo
              Abastecimento
            </h3>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                value={formData.data}
                onChange={handleChange}
                className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white font-mono focus:border-emerald-500 focus:outline-none"
              />
            </div>
            ...
            <button
              type="submit"
              className="w-full mt-6 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-colors"
            >
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-[#111827] border border-red-900/50 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-fade-in-up">
        ...
        <div className="p-5 overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-xl font-bold text-slate-100 flex items-center">
              <Wrench className="text-red-500 mr-2" size={24} /> Nova Despesa
            </h3>
            <button
              onClick={onClose}
              className="p-1 text-slate-400 hover:text-white bg-slate-800 rounded-full"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                value={formData.data}
                onChange={handleChange}
                className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white font-mono focus:border-red-500 focus:outline-none"
              />
            </div>
            <div>
              <label className="text-xs text-slate-400 ml-1">
              ...
              <input
                ...
                className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-red-500 focus:outline-none"
              />
            </div>
            <div>
              <label className="text-xs text-slate-400 ml-1">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows="2"
                className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white text-sm focus:border-red-500 focus:outline-none resize-none"
              ></textarea>
            </div>
            <button
              type="submit"
              className="w-full mt-6 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl shadow-[0_0_15px_rgba(220,38,38,0.3)] transition-colors"
            >
              Salvar Despesa
            </button>
          </form>
...
function ModalOdometro({
  ...
  const handleSubmit = (e) => {
    ...
    if (isNaN(val) || val <= config.odometro) {
      setErrorMsg("O novo odômetro deve ser maior que o atual.");
      return;
    }
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-[#111827] border border-purple-900/50 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-fade-in-up">
        ...
        <div className="p-5 overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-xl font-bold text-slate-100 flex items-center">
              <MapPin className="text-purple-500 mr-2" size={24} /> Atualizar
              Odômetro
            </h3>
            <button
              onClick={onClose}
          ...
          <p className="text-xs text-slate-400 mb-4">
            Atualize sua quilometragem para descontar o combustível gasto
            automaticamente.
          </p>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white font-mono focus:border-purple-500 focus:outline-none"
              />
            </div>
            <div>
              <label className="text-xs text-slate-400 ml-1">
              ...
              <input
                ...
                value={novoOdometro}
                onChange={(e) => {
                  setNovoOdometro(e.target.value);
                  setErrorMsg("");
                }}
                placeholder={`Atual: ${config.odometro} km`}
                className={`w-full mt-1 bg-slate-900 border rounded-xl p-3 text-white font-mono focus:outline-none transition-colors ${
                  errorMsg
            ...
            {novoOdometro &&
              !isNaN(parseFloat(novoOdometro)) &&
              parseFloat(novoOdometro) > config.odometro && (
                <div className="bg-purple-900/20 border border-purple-500/30 rounded-xl p-3 animate-fade-in-up">
                  ...
                  <span className="block text-sm font-bold text-slate-200 mt-1">
                    +{parseFloat(novoOdometro) - config.odometro} km rodados
                  </span>
            ...
            <button
              type="submit"
              className="w-full mt-6 bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-xl shadow-[0_0_15px_rgba(147,51,234,0.3)] transition-colors"
            >
              Confirmar Rota
            </button>
          </form>
